...
public static class ItemInfoSerializer
{
    ...
    public static int GetItemCntInInv((int uniqueId, string itemId)[] slots)
    {
        int itemCnt = 0;
        for (int i = 0; i < slots.Length; i++)
        {
            //특정 슬롯의 uniqueId가 0보다 크면, 즉 아이템인 경우
            if (slots[i].uniqueId > 0)
            {
                itemCnt++;
            }
    ...
    public static int GetItemCostAvg((int uniqueId, string itemId)[] slots)
    {
        int sum = 0, cnt = 0;
        for (int i = 0; i < slots.Length; i++)
        {
    ...
    public static int FindDuplicatedItem((int uniqueId, string itemId)[] slots, string itemId)
    {
        int cnt = 0;
        for (int i = 0; i < slots.Length; i++)
        {
